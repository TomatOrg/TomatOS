...
public struct FontBlitter
{
    ...
    private void DrawInternal(ReadOnlySpan<uint> pixels, Span<uint> memory, in Glyph glyph, float x, float y, uint color)
    {
        ...
        int colr = (int)(color & 0xFF), colg = (int)((color >> 8) & 0xFF), colb = (int)((color >> 16) & 0xFF);
        ...
        for (int yy = 0; yy < pbInt.Height; yy++)
        {
            ...
            for (int xx = 0; xx < pbInt.Width; xx++)
            {
                ...
                bool xCheck = (xPx >= 0) && (xPx < ab.Width);
                bool x1Check = ((xPx + 1) >= 0) && ((xPx + 1) < ab.Width);

                // Get the RGB components of the four pixels
                uint p00 = (xCheck && yCheck) ? atlas[off + xPx] : 0;
                
                uint p10 = (x1Check && yCheck) ? atlas[off + xPx + 1] : 0;
                uint p01 = (xCheck && y1Check) ? atlas[off + fullAtlasWidth + xPx] : 0;
                uint p11 = (x1Check && y1Check) ? atlas[off + fullAtlasWidth + xPx + 1] : 0;
                ...
                int val = (_screenPxRange * (a - 128) / 256 + 128);
                
                uint old = memory[(pbInt.X + xx) + (pbInt.Y + yy) * _width];
                int oldr = (int)(old & 0xFF), oldg = (int)((old >> 8) & 0xFF), oldb = (int)((old >> 16) & 0xFF);    
                
                uint newr = (uint)(oldr + (colr - oldr) * val / 256);            
                uint newg = (uint)(oldg + (colg - oldg) * val / 256);      
                uint newb = (uint)(oldb + (colb - oldb) * val / 256);
                        
                memory[(pbInt.X + xx) + (pbInt.Y + yy) * _width] = newr | (newg << 8) | (newb << 16);
            }
        }
    }
