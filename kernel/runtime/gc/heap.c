...
#define UNIT 64
STATIC_ASSERT((UNIT >> 1) < sizeof(gc_object_t));
STATIC_ASSERT(UNIT >= sizeof(gc_object_t));

typedef struct heap_rank {
    _Atomic(gc_object_t*) chunks;
    size_t freed;
} heap_rank_t;
 ...
 */
static THREAD_LOCAL gc_object_t* m_rank_head[7] = {0 };
 ...
 */
static gc_object_t* heap_rank_allocate_chunk(size_t rank, size_t count) {
 ...
 */
static gc_object_t* heap_rank_alloc(heap_rank_t* heap, size_t rank, size_t count) {
    // attempt to take a chunk from the existing heap
    gc_object_t* p = atomic_load_explicit(&heap->chunks, memory_order_acquire);
...
static void heap_rank_return(heap_rank_t* heap, size_t rank) {
    ...
    gc_object_t* o = m_rank_head[rank];
 ...
 * the global rank
 */
static void heap_rank_free(heap_rank_t* heap, size_t rank, size_t count, gc_object_t* obj) {
    // queue it
 ...
 */
static gc_object_t* heap_allocate_from(heap_rank_t* heap_rank, size_t rank, size_t count) {
    gc_object_t* p = heap_rank_alloc(heap_rank, rank, count);
 ...
 */
static gc_object_t* heap_allocate(heap_rank_t* heap_rank, size_t rank, size_t count) {
    gc_object_t* p = m_rank_head[rank];
    if (p == NULL) {
...
static gc_object_t* heap_allocate_0() { return heap_allocate(&m_heap_ranks[0], 0, HEAP_RANK_0_COUNT); }
static gc_object_t* heap_allocate_1() { return heap_allocate(&m_heap_ranks[1], 1, HEAP_RANK_1_COUNT); }
static gc_object_t* heap_allocate_2() { return heap_allocate(&m_heap_ranks[2], 2, HEAP_RANK_2_COUNT); }
static gc_object_t* heap_allocate_3() { return heap_allocate(&m_heap_ranks[3], 3, HEAP_RANK_3_COUNT); }
static gc_object_t* heap_allocate_4() { return heap_allocate(&m_heap_ranks[4], 4, HEAP_RANK_4_COUNT); }
static gc_object_t* heap_allocate_5() { return heap_allocate(&m_heap_ranks[5], 5, HEAP_RANK_5_COUNT); }
static gc_object_t* heap_allocate_6() { return heap_allocate(&m_heap_ranks[6], 6, HEAP_RANK_6_COUNT); }

/**
 * Allocate a large object, used when the object is larger than any
 * of the pre-made heaps
 */
static gc_object_t* heap_allocate_large(size_t size) {
    gc_object_t* o = palloc(size);
    ASSERT(o != NULL);
    o->rank = -1;
    return o;
}
 ...
 */
static gc_object_t* heap_allocate_medium(size_t size) {
