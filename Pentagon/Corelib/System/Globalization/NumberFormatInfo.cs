...
namespace System.Globalization
{
    ...
    public sealed class NumberFormatInfo : IFormatProvider, ICloneable
    {
        ...
        private static void VerifyDecimalSeparator(string decSep, string propertyName)
        {
            ...
            if (decSep.Length == 0)
            {
                throw new ArgumentException("Decimal separator cannot be the empty string.", propertyName);
            }
        }
        ...
        private static void VerifyNativeDigits(string[] nativeDig, string propertyName)
        {
            if (nativeDig == null)
            {
                throw new ArgumentNullException(propertyName, ArgumentNullException.Array);
            }

            if (nativeDig.Length != 10)
            {
                throw new ArgumentException("The NativeDigits array must contain exactly ten members.", propertyName);
            }
            ...
            for (int i = 0; i < nativeDig.Length; i++)
            {
                if (nativeDig[i] == null)
                {
                    throw new ArgumentNullException(propertyName, ArgumentNullException.ArrayValue);
                }
                ...
                if (nativeDig[i].Length != 1)
                {
                    if (nativeDig[i].Length != 2)
                    {
                        ...
                        throw new ArgumentException(ArgumentException.InvalidNativeDigitValue, propertyName);
                    }
        ...
        private static void VerifyDigitSubstitution(DigitShapes digitSub, string propertyName)
        {
            switch (digitSub)
            {
                ...
                default:
                    throw new ArgumentException("The DigitSubstitution property must be of a valid member of the DigitShapes enumeration. Valid entries include Context, NativeNational or None.", propertyName);
            }
        }
        ...
        internal static void CheckGroupSize(string propName, int[] groupSize)
        {
            for (int i = 0; i < groupSize.Length; i++)
            {
                if (groupSize[i] < 1)
                {
                    ...
                    throw new ArgumentException(ArgumentException.InvalidGroupSize, propName);
                }
            ...
            set
            {
                ...
                if (value < 0 || value > 11)
                {
                    throw new ArgumentOutOfRangeException(
                        nameof(value),
                        value,
                        "Valid values are between 0 and 11, inclusive.");
                }
