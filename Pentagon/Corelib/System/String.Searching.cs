...
#pragma warning restore CA2249
        ...
        public unsafe int IndexOf(char value, int startIndex, int count)
        {
            ...
            if ((uint)count > (uint)(Length - startIndex))
                throw new ArgumentOutOfRangeException(nameof(count), ArgumentOutOfRangeException.Count);
        ...
        private unsafe int IndexOfCharArray(char[] anyOf, int startIndex, int count)
        {
            ...
            ref var charMap = ref Unsafe.As<ProbabilisticMap, uint>(ref map);

            InitializeProbabilisticMap(ref charMap, anyOf);

            ref var chars = ref _firstChar;
            ref var ch = ref Unsafe.Add(ref chars, startIndex);

            while (count > 0)
            {
                int thisChar = ch;

                if (IsCharBitSet(ref charMap, (byte)thisChar) &&
                    IsCharBitSet(ref charMap, (byte)(thisChar >> 8)) &&
                    ArrayContains((char)thisChar, anyOf))
                {
                    return (int)(Unsafe.ByteOffset(ref chars, ref ch) / sizeof(char));
                }
        ...
        // inside this block.
        private static unsafe void InitializeProbabilisticMap(ref uint charMap, ReadOnlySpan<char> anyOf)
        {
            bool hasAscii = false;
            ref uint charMapLocal = ref charMap; // https://github.com/dotnet/runtime/issues/9040
            ...
            for (int i = 0; i < anyOf.Length; ++i)
            {
                ...
                // Map low bit
                SetCharBit(ref charMapLocal, (byte)c);
        ...
        {
            return (Unsafe.Add(ref charMap, (nint)value & PROBABILISTICMAP_BLOCK_INDEX_MASK) & (1u << (value >> PROBABILISTICMAP_BLOCK_INDEX_SHIFT))) != 0;
        }

        private static void SetCharBit(ref uint charMap, byte value)
        {
            Unsafe.Add(ref charMap, (nint)value & PROBABILISTICMAP_BLOCK_INDEX_MASK) |= 1u << (value >> PROBABILISTICMAP_BLOCK_INDEX_SHIFT);
        }
            ...
            switch (comparisonType)
            {
                ...
                default:
                    throw (value is null)
                        ...
                        : new ArgumentException(ArgumentException.StringComparison, nameof(comparisonType));
            }
        }
