...
namespace System
{
    public partial class Random
    {
        ...
        internal sealed class XoshiroImpl : ImplBase
        {
            ...
            public XoshiroImpl()
            {
                // Used the TSC as a base measurement
                // TODO: do we maybe want a better random seed source?
                var seed = (ulong)Stopwatch.GetTimestamp();
                
                // take the seed and mix it into a 256bit seed
                do
                {
            ...
            public XoshiroImpl(int seed)
            {
                // turn the seed into a ulong with splitmix32 
                var useed = (uint)seed;
                var lseed = NextSplitMix32(ref useed) | ((ulong)NextSplitMix32(ref useed) << 32);
                
                // take the seed and mix it into a 256bit seed
                do
                {
                    _s0 = NextSplitMix64(ref lseed);
                    _s1 = NextSplitMix64(ref lseed);
                    _s2 = NextSplitMix64(ref lseed);
                    _s3 = NextSplitMix64(ref lseed);
                } while ((_s0 | _s1 | _s2 | _s3) == 0);
            }
            ...
            public override unsafe void NextBytes(Span<byte> buffer)
            {
                ...
                if (!buffer.IsEmpty)
                {
                    ...
                    for (int i = 0; i < buffer.Length; i++, next >>= 8)
                    {
                        buffer[i] = (byte)next;
                    }
