...
public class Stack : Widget
{
    ...
    public Stack(Widget[] children, StackAlign align = StackAlign.Center, StackFit fit = StackFit.Expand)
    {
        _children = children;
            ...
            for (var i = 0; i < _builtChildren.Length; i++)
            {
                if (childrenFlexX[i] == 0 || childrenFlexY[i] == 0)
                {
                    childrenSizes[i] = _builtChildren[i]
                        .Layout(0, 0, long.MaxValue, long.MaxValue);
                }
                else
                {
                    childrenSizes[i] = (0, 0);
